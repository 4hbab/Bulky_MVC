...
namespace BulkyWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null) {
            // # Adding Roles
            // > Zie Bulky.Utility
            // > .GetAwaiter().GetResult() > Is hetzelfde als vooraan "await" schrijven
            if (_roleManager.RoleExistsAsync(SD.Role_Customer).GetAwaiter().GetResult() == false) {
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Customer)).GetAwaiter().GetResult();
            }
            if (_roleManager.RoleExistsAsync(SD.Role_Employee).GetAwaiter().GetResult() == false) {
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Employee)).GetAwaiter().GetResult();
            }
            if (_roleManager.RoleExistsAsync(SD.Role_Admin).GetAwaiter().GetResult() == false) {
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Admin)).GetAwaiter().GetResult();
            }
            if (_roleManager.RoleExistsAsync(SD.Role_Company).GetAwaiter().GetResult() == false) {
